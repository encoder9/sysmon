#!/usr/bin/perl
#
# System Monitor - Local Storage Only - Created by Bradley J. Gibby :)
...
my $outputData = 0;
my %payload = ();
my $outputPath = './dataFiles/';

print "System Monitor - Local Storage Only - Created by Bradley J. Gibby :)\n" if ($outputData);
...
print FH $ts . ':' . $payload{uptime}{la1} . "\n";
close(FH);

# Load Avg 5M
open(FH, '>>' . $outputPath . 'la5.log');
print FH $ts . ':' . $payload{uptime}{la5} . "\n";
close(FH);

# Load Avg 15M
open(FH, '>>' . $outputPath . 'la15.log');
print FH $ts . ':' . $payload{uptime}{la15} . "\n";
close(FH);
